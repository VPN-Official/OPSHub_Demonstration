...
class Asset(UUIDModel, TimeStampedModel, TenantScopedModel):
...
class AssetConfigurationItem(UUIDModel, TimeStampedModel, TenantScopedModel):
    asset = models.ForeignKey(Asset, related_name="configuration_items", on_delete=models.CASCADE)
    key = models.CharField(max_length=100)
    value = models.CharField(max_length=255)

class AssetComplianceCertificate(UUIDModel, TimeStampedModel, TenantScopedModel):
    asset = models.ForeignKey(Asset, related_name="compliance_certificates", on_delete=models.CASCADE)
    certificate_type = models.CharField(max_length=100)
    expiry_date = models.DateField()
    status = models.CharField(max_length=50)

class AssetCostTracking(UUIDModel, TimeStampedModel, TenantScopedModel):
